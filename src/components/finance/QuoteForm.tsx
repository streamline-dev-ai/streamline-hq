...
import { Button, Sheet, Field, Select, Textarea } from "@/ui";
import LineItemsEditor from "./LineItemsEditor";
import { computeTotals, zar, DEFAULT_VAT_RATE, type Client, type LineItem } from "@/lib/finance";
...
export default function QuoteForm({
  ...
  const [validUntil, setValidUntil] = useState(addDaysToSaYmd(getSaDateString(), 14));
  ...
  useEffect(() => {
    ...
    setValidUntil(addDaysToSaYmd(getSaDateString(), 14));
    setVatEnabled(false);
  ...
  const totals = useMemo(() => computeTotals(items, vatEnabled, vatRate), [items, vatEnabled, vatRate]);

  async function save() {
    if (!clientId) return pushToast({ type: "error", title: "Quote", message: "Pick a client" });
    const clean = items.filter((i) => i.description.trim() || i.unit_price);
    if (clean.length === 0)
      return pushToast({ type: "error", title: "Quote", message: "Add at least one line item" });
    setSaving(true);
    try {
      ...
      const quoteId = (ins.data as { id: string }).id;
      ...
      const li = await supabase.from("finance_quote_line_items").insert(rows);
      if (li.error) throw li.error;
      pushToast({ type: "success", title: "Draft created", message: "Quote" });
      onSaved();
      onClose();
    } catch (e) {
  ...
  return (
    <Sheet open={open} onClose={onClose} title="New quote">
      <div className="space-y-4">
        ...
        <div>
          <div className="mb-2 text-xs font-medium uppercase tracking-wide text-ink-faint">Line items</div>
        ...
        <div className="rounded-xl border border-line bg-surface p-3">
          ...
          >
            ...
            >
              <span className={`h-5 w-5 rounded-full bg-white transition ${vatEnabled ? "translate-x-5" : ""}`} />
            </span>
          </button>
          {vatEnabled && (
            <div className="mt-3">
        ...
        <div className="space-y-1 rounded-xl border border-line bg-base/40 p-3 text-sm">
          <div className="flex justify-between text-ink-muted">
            <span>Subtotal</span>
            <span className="font-mono tabular-nums">{zar(totals.subtotal)}</span>
          </div>
          {vatEnabled && (
            <div className="flex justify-between text-ink-muted">
              <span>VAT ({vatRate}%)</span>
              <span className="font-mono tabular-nums">{zar(totals.vat_amount)}</span>
            </div>
          )}
